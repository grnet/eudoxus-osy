...
{
    public class GenerateTransferPerIssuerExcel : BaseHttpHandler
    {
        ...
        protected override void DoProcessRequest()
        {
            if (OfficeSlipDate != null && OfficeSlipDate > DateTime.MinValue && SupplierKpsID > 0)
            {
                ...
                Response.AddHeader("Content-Disposition",
                    string.Format("attachment; filename=TransferPerIssuer-{0}_{1}.xls",
        ...
        {
            localReport.ReportPath = HttpContext.Current.Server.MapPath("~/_rdlc/TransferPerIssuer.rdlc");
            ...
            var totalDeductionIncomeTax = Math.Round(totalInvoiceValue * IncomeTaxPerc, 2);
            ...
            ReportDataSource ds = new ReportDataSource("TransferPerIssuer", oss.ToDataTable());
        ...
        private List<OfficeSlipSupplier> ProcessGroup(CatalogGroup catalogGroup, out OfficeSlipSupplier totalCalculatedAmounts)
        {
            ...
            foreach (Invoice invoice in catalogGroup.Invoices)
            {
                ...
                if (hasLogisticBooks)
                {
                    var IncomeTaxAmount = Math.Round(invoice.InvoiceValue * IncomeTaxPerc, 2);
                ...
                else
                {
                    ...
                    var IncomeTaxAmount = Math.Round(invoice.InvoiceValue * IncomeTaxPerc, 2);
                    var TotalDeductionsAmount = Math.Round(deductionStamp + deductionOGA + IncomeTaxAmount, 2);
                ...
                if (groupCounter == count)
                {
                    ...
                    if (hasLogisticBooks)
                    {
                        ...
                        var IncomeTaxAmount = Math.Round((decimal)catalogGroup.InvoiceSum.Value * IncomeTaxPerc, 2);

                        var TotalDeductionsAmount = IncomeTaxAmount;
                        decimal TotalVatAmount = 0m;
                        if (catalogGroup.Deduction != null)
                        {
                            TotalVatAmount = Math.Round((catalogGroup.Deduction.Vat / 100) * (decimal)catalogGroup.InvoiceSum.Value, 2);
                        }
                        else
                        {
