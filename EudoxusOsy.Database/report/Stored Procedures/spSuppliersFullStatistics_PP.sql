...
totalpayment,totalofferprice,totaltoyde,status,phase_id,booksPriced,booksNotPriced)
SELECT cp.SupplierID,s.SupplierKpsID,s.Name,s.PaymentPfo,s.Email,s.AFM,s.TradeName, s.SupplierType, cp.totalMoney,
cp.givenMoney, ISNULL(cp.totalMoney,0) - ISNULL(cp.givenMoney,0),gp.sentMoney,1, cp.PhaseID, cp.pricedBooks, cp.notPricedBooks
    FROM (
			SELECT catalog.SupplierID,
					catalog.PhaseID,
					ROUND(SUM(ISNULL(catalog.Amount,0)), 2) AS totalMoney,
				...
				WHERE catalog.Status = 1 AND catalog.State IN (0,1) AND PP.IsActive = 1						
    ...
    LEFT JOIN (
			SELECT catalog.SupplierID,
					catalog.PhaseID,
					ROUND(SUM(ISNULL(Amount,0)), 2) AS sentMoney
				FROM Catalog
				JOIN Phase PP on PP.ID = catalog.PhaseID and PP.ID <> @currentPhaseID
	...
	--EXEC [report].[spSuppliersFullStatistics_PP] 
