USE [EudoxusOsy-App]
GO
/****** Object:  StoredProcedure [dbo].[CreateCatalogsForPhase]    Script Date: 17/3/2019 2:45:12 μμ ******/
SET ANSI_NULLS ON
GO
...
Begin
	...
	BEGIN  	
		...
		Begin			
			...
			IIF(
				IIF(
					IIF(@diff_years - 7 > 0, @diff_years - 7, 0) > 5, 5, 
					IIF(@diff_years - 7 > 0, @diff_years - 7, 0))
					 > 5, 
					 5,
				IIF(
					IIF(@diff_years - 7 > 0, @diff_years - 7, 0) > 5, 5, 
	...
	UPDATE cat SET HasPendingPriceVerification = 1
	FROM Catalog cat
	JOIN Book b ON b.ID = cat.BookID
	WHERE b.PendingCommitteePriceVerification = 1 AND cat.PhaseID = @myPhaseID

	UPDATE cat SET HasUnexpectedPriceChange = 1
	FROM Catalog cat
	JOIN Book b ON b.ID = cat.BookID
	WHERE b.HasUnexpectedPriceChangePhaseID = @myPhaseID
