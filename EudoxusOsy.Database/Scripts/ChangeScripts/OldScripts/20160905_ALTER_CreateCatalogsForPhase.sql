...
Begin
	...
	-- Reduce price for 2% for every year after 16 semesters from FirstRegistration until 10% is reached
		...
		BEGIN
			...
			Begin
				...
				-- We must first fill the database with the discount levels for the new year, in order for the following part to work 
				if @BookDiscountID = 0
				Begin
					Select @BookDiscountID = ISNULL(ID,0), @BookDiscountFrom = BookCountFrom, @BookDiscountTo = BookCountTo, @BookDiscountPercentage = ISNULL(DiscountPercentage, -1)
